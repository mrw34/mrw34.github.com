...
- The container was not immediately rebuildable due to a missing file (```subread-1.4.6-p2-Linux-x86_64.tar.gz```). It was possible to search the web for this and retrieve this manually. The container could then be rebuilt and run, and the provided notebook executed within the provided Jupyter server (based on the SciPy stack, as required).
...
- The notebook did not run to completion as it assumed the presence of data files for a reference genome (```genomes/Homo_sapiens/UCSC/hg19```) that were missing. It wasn’t clear from where this data could be retrieved.
...
- Link to repositories rather than directly to rendered notebooks if they depend on external files
...
- If you require a large amount of data, then make it clear where this should be retrieved from, and provide clear instructions on how to mount this from the local drive to the container.
- Ensure that you double-check that your notebook/container can be run on a clean system.

## Next steps

This was a one-day project, which leave lots of potential work to be done. Here are some ideas:
